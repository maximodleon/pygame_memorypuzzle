...
FPS = 30 # frames per second, the speed of the game
...
def main():
    ...
    firstSelection = None  # srotes the (x, y) of the first box clicked
...
def getRandomizedBoard():
    ...
    numIconUsed = int (BOARDWIDTH * BOARDHEIGHT / 2) #calculate how many icons
    # are needed
...
def splitIntoGroupsOf(groupSize, theList):
    # splits a list into a list of lists, wher the inner lists have at most
...
def leftTopCoordOfBox (boxx, boxy):
    # convert the board of coordinates to pixel coordinates
...
def drawIcon (shape, color, boxx, boxy):
    ...
    left, top = leftTopCoordOfBox(boxx, boxy) # get pixel coords from board coords
    # draw the shapes
    if shape == DONUT:
...
def drawBoxCovers(board, boxes, coverage):
    ...
    # of two-item lists, which the x & y spot of the box
    for box in boxes:
...
def drawHighlightBox(boxx, boxy):
    left, top = leftTopCoordOfBox(boxx, boxy)
    pygame.draw.rect (DISPLAYSURF, HIGHLIGHTCOLOR, (left - 5, top - 5, BOXSIZE + 10 \
    , BOXSIZE + 10), 4)
...
def gameWonAnimation(board):
    # flash the background color when the player won
...
def hasWon(revealedBoxes):
    # returns True if all the boxes have been revealed, othewise False
    for i in revealedBoxes:
        if False in i:
            return False # return False if any of the boxes is covered
    return True
